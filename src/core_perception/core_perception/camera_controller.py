...
class CameraController(Node):
    ...
    def __init__(self):
        ...
        self.up_camera_serial_idx = get_webcam_device_idx(
            "046d_C270_HD_WEBCAM_E0198440"
        )
        self.down_camera_serial_idx = get_webcam_device_idx(
        ...
        if self.REC_UP_CAMERA:
            ...
            file_name = "core_perception/videos/" + time.strftime(
                "%d-%m-%Y_%H:%M:%S_up_cam.mp4", time.localtime()
            )
            self.up_vid_writer = cv2.VideoWriter(
                file_name, cv2.VideoWriter_fourcc(*"mp4v"), fps, size
            )
        if self.REC_DOWN_CAMERA:
            fps = 15
            size = int(self.down_cap.get(3)), int(self.down_cap.get(4))
            file_name = "core_perception/videos/" + time.strftime(
                "%d-%m-%Y_%H:%M:%S_down_cam.mp4", time.localtime()
            )
            self.down_vid_writer = cv2.VideoWriter(
                file_name, cv2.VideoWriter_fourcc(*"mp4v"), fps, size
            )
        ...
        self.get_logger().info(
            f"<> [{NodeConfig.camera_front}] Successfully initialized node"
        )
    ...
    def _take_upper_photo(self):
        ...
        if ret:
            ...
            photo_filename = (
                f"/home/amv/KKI-25/core_perception/photos/{timestamp}_upCamera.jpg"
            )
            cv2.imwrite(photo_filename, frame)
            self.get_logger().info(
                f"Photo taken and saved to {photo_filename}", throttle_duration_sec=5.0
            )
    ...
    def _take_down_photo(self):
        ...
        if ret:
            timestamp = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
            photo_filename = (
                f"/home/amv/KKI-25/core_perception/photos/{timestamp}_downCamera.jpg"
            )
            cv2.imwrite(photo_filename, frame)
            self.get_logger().info(
                f"Photo taken and saved to {photo_filename}", throttle_duration_sec=5.0
    ...
    def mission_callback(self, msg):
        ...
        self.get_logger().info(
            f"Mission changed to: {self.mission_type}", throttle_duration_sec=3.0
        )
    ...
    def process_frame(self):
        ...
        try:
            ...
            if self.mission_type == MissionStatus.BUOY:
                self.img, self.dsc, self.detected = self.buoy_detector.process_frame(
                    self.mission_type, self.arena, img, self.up_cap, self
                )
            else:
                self.get_logger().info("Masuk sini", throttle_duration_sec=1.0)
                self.img, self.dsc, self.detected = self.box_detector.process_frame(
                    self.mission_type, self.arena, img, self.up_cap, self
                )
            self.get_logger().info(
                f"Test: {self.mission_type}", throttle_duration_sec=1.0
            )
