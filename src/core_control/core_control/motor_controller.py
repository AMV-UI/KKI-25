...
class MotorController(Node):
    ...
    def _setup_communication(self):
        ...
        # Subscribers
        self.yaw_effort_sub = Topic.yaw_effort.createSubscriber(
            self, self._yaw_effort_callback
        )
        self.speed_effort_sub = Topic.speed_effort.createSubscriber(
            self, self._speed_effort_callback
        )
    ...
    def display_pwm_status(self):
        ...
        self.get_logger().info(
            f"  - Range: {status['pwm_range']['min']} to {status['pwm_range']['max']}"
        )
    ...
    def autonomous(self):
        ...
        for k, v in self.motor.autonomous(
            control_effort_x=self.yaw_effort, control_effort_y=self.speed_effort
        ).items():
    ...
    def run(self):
        ...
        self.get_logger().info(
            f"<> [{NodeConfig.motor_controller}] Successfully initialized node"
        )
    ...
    def test_sequence(self, timer):
        ...
        try:
            ...
            self.get_lmodeogger().error(
                f"Error during motor test sequence: {traceback.format_exc()}"
            )
