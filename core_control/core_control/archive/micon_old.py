...
from kki24.msg import Pwm, AutoControl, KillSwitch, Pixhawk
...
class Microcontroller:
    def __init__(self):
        ...
        rospy.loginfo_once(f"<> [{Node.microcontroller}] PWM Subscriber created")
    ...
    def _init_mc(self):
        ...
        try:
            if dirs[0] != "/dev/ttyUSB0":
                ...
                # self.ser_2= mavutil.mavlink_connection("/dev/ttyUSB0", baud=57600)
                rospy.loginfo_once(
                    f"<> [{Node.microcontroller}] Waiting for px initiated"
                )
                self.ser_2.mav.heartbeat_send(0, 0, 0, 0, 0)
            ...
            rospy.logerr_throttle(
                5, f"<> [{Node.microcontroller}] MC Index is not found"
            )
            # print(e)
            self.mc1 = MiconType.NONE
        ...
        if rc_channels.chan8_raw > 1700:
            try:
                for pwm_val in self.pwm_chan.channels:
                    ...
                    self.set_rc_channel_pwm(pwm_count, pwm=int(pwm_val))
                    rospy.logerr_throttle(5, "<=> [{Node.microcontroller}] PWM sent")
            ...
            try:
                for pwm_val in self.pwm_chan.channels:
                    ...
                    rospy.logerr_throttle(5, "<=> [{Node.microcontroller}] Change mode to Manual control. Throttle PWM.")
                    pwm_count = pwm_count + 1
                    # time.sleep(0.2)
            except Exception as e:
                rospy.logerr(
            ...
            rospy.logerr_throttle(5, "<=> [{Node.microcontroller}] Error Mode : PWM not sent")
    ...
    def _px_arm(self):
        ...
        rospy.logerr_throttle(5, f"<=> [{Node.microcontroller}] Arming motors ...")
        self.ser_2.motors_armed_wait()
        rospy.logwarn_once(f"<=> [{Node.microcontroller}] Motor Armed!")
    ...
    def _px_set_mode(self, pwm_val):
        ...
        rospy.loginfo(f'<=> [{Node.microcontroller}] Current Mode : {self.pxmode}')

        if self.pxmode not in self.ser_2.mode_mapping():
            rospy.logwarn_once(f"<=> [{Node.microcontroller}] Unknown Mode : {self.pxmode}")
        ...
        rospy.loginfo(f'[{Node.microcontroller}] Mode set to : {self.pxmode}')  
        return True
    ...
    def main(self):
        # Publisher
        kill_switch_pub = Topic.kill_switch.createPublisher()
        heading_deg_pub = Topic.heading_deg.createPublisher()
        auto_status_remote_pub = Topic.auto_status_remote.createPublisher()
        jetson_batt_pub = Topic.jetson_batt.createPublisher()
        motor_batt_pub = Topic.motor_batt.createPublisher()
        mux_state_pub = Topic.mux_state.createPublisher()
        pixhawk_pub = Topic.pixhawk.createPublisher()
        pxmode_pub = Topic.pxmode.createPublisher()
        ...
        while not rospy.is_shutdown():
            ...
            rospy.logwarn_once(f"<=> [{Node.microcontroller}] micon found")
            ...
            # self._get_pwm()
            rospy.logwarn_throttle(5, f"<=> [{Node.microcontroller}] Sending PWM...")
...
if __name__ == "__main__":
    try:
        rospy.init_node(Node.microcontroller)
