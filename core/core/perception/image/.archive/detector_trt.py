...
class TrtYOLO(object):
    ...
    def _load_engine(self):
        TRTbin = f"{self.model}/yolo.engine"
        with open(TRTbin, 'rb') as f, trt.Runtime(self.trt_logger) as runtime:
            return runtime.deserialize_cuda_engine(f.read())

    def __init__(self, model, threshold = 0.8, cuda_ctx=None):
        ...
        self.classes_names, self.inv_classes = self._read_class_names(self.model + '/classes.names')
    ...
    def predict(self, img):
        ...
        boxes, scores, classes = _postprocess_yolo(
            trt_outputs, img.shape[1], img.shape[0], self.threshold,
            nms_threshold=0.5)
    ...
    def draw(self, frame, boxes, confs, clss):
        for bb, cf, cl in zip(boxes, confs, clss):
            ...
            label = str(self.classes_names[cl])
            confidence = str(round(cf, 2))
            cv2.putText(frame, label + " - " + confidence,
                        (x_min, y_min-10), 0, 0.75, (255, 255, 255), 2)
