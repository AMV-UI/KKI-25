...
from core.utils.config import Box, Camera, NodeConfig, Topic, ModelPath, Tower
...
class BottomCamera:
    ...
    def do_capture(self):
        ...
        try:
            ...
            rospy.logerr_throttle(5, f"<=> [{NodeConfig.camera_bottom}] Captured frame")
            return encoded_image
            # self.fps_counter.calculate(frame)
            # self.camera_processed_pub.publish(compressImage(frame))
        except Exception as e:
            rospy.logerr_throttle(5, f"<=> [{NodeConfig.camera_bottom}] Improc error,", e)
